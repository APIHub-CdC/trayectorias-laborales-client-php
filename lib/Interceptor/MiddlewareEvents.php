...
namespace tl\mx\Client\Interceptor;
...
use \tl\mx\Client\Interceptor\MyLogger;
use \tl\mx\Client\Model\Errors;
use \tl\mx\Client\Model\Error;

class MiddlewareEvents
{
    
    function __construct(\tl\mx\Client\Interceptor\KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    $error = new \tl\mx\Client\Model\Error([
        "code" => $code,
        "message" => $message
    ]);
    $errors = new \tl\mx\Client\Model\Errores(["errors" => [$error]]);
